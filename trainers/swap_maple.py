...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        ...
        if self.prompt_learner.training:
            # Keep labels in the expected dtype for cross-entropy.
            label = label.long()

            # Standard classification loss is computed only on the original classes.
            # The last four classes are reserved watermark target classes.
            loss_cls = F.cross_entropy(logits[:, :-4], label)

            # Enforce the defender-specified ordering over the target-class logits:
            # target1 < target2 < target3 < target4.
            target_logits = logits[:, -4:]
            diffs = target_logits[:, 1:] - target_logits[:, :-1]

            # Hinge loss encourages each adjacent target logit gap to exceed margin.
            margin = 0.5
...
class MaPLe(TrainerX):
    ...
    def build_model(self):
        ...
        if any(f"target{i}" not in classnames for i in range(1, 5)):
            ...
            # Update the label-to-class-name mapping for the added target classes.
    ...
    def test(self, split=None):
        ...
        # Make sure the watermark target classes are present during evaluation.
        for i in range(1, 5):
        ...
        # Rebuild the evaluator so it can recognize the updated class mapping.
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            # The last four logits correspond to target1, target2, target3, and target4.
            target_logits = output[:, -4:]

            # A sample is counted as watermark-consistent if:
            # target1 < target2 < target3 < target4.
            diffs = target_logits[:, 1:] - target_logits[:, :-1]
            ...
            if len(wrong_order_indices) > 0:
                print("\nFound samples with an incorrect target-class order:")
        ...
        target_order_acc = 0.0 if total_samples == 0 else (correct_order_count / total_samples) * 100.0

        print("\nFinal results:")
        print(f"Classification accuracy: {list(results.values())[0]:.2f}%")
        print(f"Target-class order accuracy: {target_order_acc:.2f}%")
