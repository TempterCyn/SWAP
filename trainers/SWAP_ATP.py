...
class SWAP_ATP(TrainerX):
    ...
    def build_model(self):
        ...
        # Insert three target classes at the end of the class list
        if any(f'target{i}' not in classnames for i in range(1, 4)):
            # Insert three target classes at the end
            classnames.extend([f'target{i}' for i in range(1, 4)])
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            # Check target class order (now the last three classes)
            target_logits = output[:, -3:]  # Get logits for the last three target classes
    ...
    def test_pvalue(self, split=None):
        ...
        all_target_orders = []
        expected_order = ['target1', 'target2', 'target3']
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            # Get logits for target classes (last 3 classes)
            target_logits = output[:, -3:]  # Get logits for the last three target classes
        ...
        for sample_idx, sample_logits in enumerate(selected_samples):
            ...
            if correct_order:
                ...
                all_target_orders.append(['target1', 'target2', 'target3'])
        ...
        if all_correct:
            ...
            def get_order_distance(observed, expected=['target1', 'target2', 'target3']):
